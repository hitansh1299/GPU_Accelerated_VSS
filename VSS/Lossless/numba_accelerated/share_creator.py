...
#Broken the function into two parts to keep the pure numba functions separate from Matplotlib utils
@jit(nopython=True, cache=True, parallel=PARALLEL)
def __generate_shares__(img: np.ndarray):
    ...
    for plane in prange(3):
        for i in prange(BITPLANE_COUNT):
            bitplane = get_bitplane(img[:,:,plane], i)
            s1, s2 = create_bitplane_shares(bitplane)
            share_image1[:,:,plane] = share_image1[:,:,plane] + (s1 * 2**i).astype(np.uint8)
            share_image2[:,:,plane] = share_image2[:,:,plane] + (s2 * 2**i).astype(np.uint8)
        
    # print("SI1",share_image1)
